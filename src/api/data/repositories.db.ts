...
import respositoriesData from "./repositories.json";
...
async function getByUserId(userId: number): Promise<Repository[]> {
  return repositories.filter((repo) => repo.owner.id == userId);
}
...
export { reset, getById, getAll, getByUserId };
